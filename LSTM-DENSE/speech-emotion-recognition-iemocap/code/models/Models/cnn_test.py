...
def load_data(args, config):
    ...
    dataset = pickle.load(open('NSC_part5_labelled_emotion/Preprocessed_filesfeature_vectors_Angry.pkl','rb'))
...
def main(args):
    ...
    valloader = load_data(args, config)
    ...
    model = CNN()
    model.to(device)
    model.load_state_dict(torch.load('best_model_session.pt'))
