...
def load_data(args, config):
    ...
    datasetee = pickle.load(open('/home/achharia/Domain_Adaptation_2/NSC_datasetset_balaanced.pkl','rb'))
    datasete = pickle.load(open('/home/achharia/Domain_Adaptation_2/domain_adaptation/LSTM-DENSE/speech-emotion-recognition-iemocap/preprocess_info/preprocess_infofeature_vectors_emodb.pkl','rb'))
    ...
    mfccsee = []
    print("Loading Dataset . . .")
    for i in range(len(datasetee['label'])):
        if datasetee['label'][i] in [0,1,3,7]:
            label = 0
            if datasetee['label'][i]==3:
                label = 2
            elif datasetee['label'][i]==7:
                label =3
            elif datasetee['label'][i]==1:
                label=1
            spectogramsee.append(datasetee['spec_db'][i])
            labelsee.append(label)
            filenamesee.append(datasetee['wav_file'][i])
            mfccsee.append(datasetee['mfccs'][i])
    
            
    datasetee['label'] = labelse
    datasetee['spec_db'] = spectogramse
    datasetee['mfccs'] = mfccse
    datasetee['wav_file'] = filenamese
    ...
    num_traine = int(len(datasete['label']) * 0.8)
    num_teste = len(datasete['label']) - num_train

    num_trainee = int(len(datasete['label']) * 0.5)
    num_testee = len(datasete['label']) - num_train
    ...
    indicesee = list(range(len(datasetee['label'])))

    random.shuffle(indices)
    random.shuffle(indicese)
    random.shuffle(indicesee)
    ...
    train_indicesee = indicesee[:num_trainee]
    test_indicesee = indicesee[num_trainee:]
    ...
    for key in dataset:
        ...
        for i in test_indicese:
            test_dataset[key].append(datasete[key][i])
        # for i in train_indicesee:
        #     train_dataset[key].append(datasetee[key][i])
        #     #print(datasete[key][i])
        # for i in test_indicesee:
        #     test_dataset[key].append(datasetee[key][i])
            # if key=="label":
        #         #print(datasete[key][i])
            #print(datasete[key][i])
...
def main(args):
    ...
    # layer1_weights = torch.load('layer1_weights.pth')
    # layer2_weights = torch.load('layer2_weights.pth')
    # layer3_weights = torch.load('layer3_weights.pth')
    # layer4_weights = torch.load('layer4_weights.pth')
    # fc1_layer = torch.load('fc1_weights.pth')
    # fc2_layer = torch.load('fc2_weights.pth')
    # fc3_layer = torch.load('fc3_weights.pth')
...
#    for i, (inputs, labels,filename) in enumerate(valloader):
    ...
    for epoch in range(1, n_epoch):    
        ...
        if test_acc > best_val_acc:
            ...
            torch.save(model.layer1.state_dict(), 'layer1_weighte.pth')
            torch.save(model.layer2.state_dict(), 'layer2_weighte.pth')
            torch.save(model.layer3.state_dict(), 'layer3_weighte.pth')
            torch.save(model.layer4.state_dict(), 'layer4_weighte.pth')
            torch.save(model.fc1.state_dict(), 'fc1_weighte.pth')
            torch.save(model.fc2.state_dict(), 'fc2_weighte.pth')
            torch.save(model.fc3.state_dict(), 'fc3_weighte.pth')
